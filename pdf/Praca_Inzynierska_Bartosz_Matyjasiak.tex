...
Dzięki zmiennej \code{mapRegion} dostępnej w komponencie \code{GMap} moge ograniczyć pinezki do wyświetlenia.
...
\begin{itemize}
  ...
  \item{\code{latitudeDelta} -- różnica szerokości geografinczych od lewej krawędzi mapy do prawej,}
  \item{\code{longitudeDelta} -- różnica długości geografinczych od górnej krawędzi mapy do dolnej.}
\end{itemize}
...
By to rozwiązać do filtracji pinezek używam czterokrotnie większego pola niż pole wyznaczone przez \code{mapRegion}
...
a klucz map Google, należało umieścić w pliku \code{AndroidManifest.xml} przy pomocy odwołania do zmiennych srodowiskowych po uprzedniej konfiguracji modułu \code{react-native-config}.
...
Teraz by było możliwe zbudowanie aplikacji w trybie debug należy wypełnić plik \code{.env.example} własnymi kluczami i zmienić jego nazwe na \code{.env}.
